...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.en_convs = nn.ModuleList([
            ConvBlock(3*3, 48, (1,5), stride=(1,2), padding=(0,2), use_deconv=False, is_last=False),
...
@BackboneRegister.register("gtcrn")
class GTCRN(nn.Module):
    def __init__(self, input_channels, discriminative=False):
        ...
        self.dpgrnn1 = DPGRNN(48, 33, 48)
        self.dpgrnn2 = DPGRNN(48, 33, 48)
    ...
    def forward(self, spec, time_cond=None):
        ...
        if not self.discriminative:
            ...
            spec = torch.stack([spec[:,0,:,:].real, spec[:,0,:,:].imag], -1)
        ...
        if not self.discriminative:
            # 时间步嵌入
            t_emb = self.time_embed(time_cond)  # (B, 128)
            # 将时间信息和噪声信息注入特征
            t_emb = t_emb.unsqueeze(-1).unsqueeze(-1)  # (B,128,1,1)
            t_emb = t_emb.expand(-1, -1, feat.shape[2], feat.shape[3])  # (B,128,T,F)
        ...
        # # 使用thop库计算MACs和参数数量
        # macs, params = profile(self.encoder, inputs=(feat,))
        # print('MACs: {:.2f} G, Params: {:.2f} M'.format(macs / 1000000000.0, params / 1000000.0))
        feat, en_outs = self.encoder(feat)

        # # 使用thop库计算MACs和参数数量
        # macs, params = profile(self.dpgrnn1, inputs=(feat,))
        # print('MACs: {:.2f} G, Params: {:.2f} M'.format(macs / 1000000000.0, params / 1000000.0))
