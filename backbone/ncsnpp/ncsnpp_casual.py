...
ResnetBlockBigGAN = layerspp.ResnetBlockCausalBigGANpp
Combine = layerspp.Combine
conv3x3 = layerspp.causal_conv3x3
...
@BackboneRegister.register("ncsnpp_base_casual")
class NCSNppCasual(nn.Module):
    """NCSN++ MISB"""
    ...
    def __init__(self, 
        ...
        **kwargs):
        ...
        AttnBlock = functools.partial(layerspp.CausalAttnBlockpp, 
            init_scale=init_scale, skip_rescale=skip_rescale)

        Upsample = functools.partial(layerspp.UpsampleCausal, 
            with_conv=resamp_with_conv, fir=fir, fir_kernel=fir_kernel)

        if progressive == 'output_skip':
            self.pyramid_upsample = layerspp.UpsampleCausal(fir=fir, fir_kernel=fir_kernel, with_conv=False)
        elif progressive == 'residual':
            pyramid_upsample = functools.partial(layerspp.UpsampleCausal, fir=fir, 
                fir_kernel=fir_kernel, with_conv=True)

        Downsample = functools.partial(layerspp.DownsampleCausal, with_conv=resamp_with_conv, fir=fir, fir_kernel=fir_kernel)

        if progressive_input == 'input_skip':
            self.pyramid_downsample = layerspp.DownsampleCausal(fir=fir, fir_kernel=fir_kernel, with_conv=False)
        elif progressive_input == 'residual':
            pyramid_downsample = functools.partial(layerspp.DownsampleCausal,
        ...
        for i_level in reversed(range(num_resolutions)):
            ...
            if progressive != 'none':
                if i_level == num_resolutions - 1:
                    if progressive == 'output_skip':
                        modules.append(layerspp.GroupLayerNorm2d(num_groups=min(in_ch // 4, 32), 
                            num_channels=in_ch, eps=1e-6, affine=True))
                        modules.append(conv3x3(in_ch, self.total_channels, init_scale=init_scale))
                        pyramid_ch = self.total_channels
                    elif progressive == 'residual':
                        modules.append(layerspp.GroupLayerNorm2d(num_groups=min(in_ch // 4, 32), num_channels=in_ch, eps=1e-6, affine=True))
...
@BackboneRegister.register("ncsnpp_tinny_tinny_tinny_tinny_casual")
class NCSNpp40KCausal(NCSNppCasual):
...
if __name__ == '__main__':
    # 26.47MB
    model = NCSNppCasual(input_channels=2)
