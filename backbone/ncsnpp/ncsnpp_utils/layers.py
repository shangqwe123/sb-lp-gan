...
class CustomPad(nn.Module):
    def __init__(self, padding):
        ...
        self.padding = padding  # 例如 (2, 0, 1, 1)

    def forward(self, x):
        # F.pad 的顺序是 (left, right, top, bottom)
        return F.pad(x, self.padding)
    
def casual_ddpm_conv3x3(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1., padding=1):
  """3x3 convolution with DDPM initialization."""
  conv = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=0, bias=bias)
  conv.weight.data = default_init(init_scale)(conv.weight.data.shape)
  if bias:
    nn.init.zeros_(conv.bias)
  return nn.Sequential(CustomPad((2, 0, 1, 1)), conv)
