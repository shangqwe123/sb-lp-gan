    ...
    with torch.no_grad():
        ...
        for _x, _audio_name in iter:
            ...
            if not os.path.exists(audio_path) or config.overwrite:
                noisy_audio_path = os.path.join(config.output_path, f'{audio_name}_noisy.wav')

                torchaudio.save(noisy_audio_path, x.type(torch.float32).cpu().squeeze().unsqueeze(0), config.data.get('sample_rate', 16000))
                if True:
...
if __name__ == '__main__':
    ...
    # Sampling Parameters
    parser.add_argument("--sampling_method", type=str, default="ddim", help="Sampling method: ddim, hybrid, etc.")
    ...
    if args.dataset in default_config.datasets.keys():
        noisy_path = os.path.join(default_config.datasets[args.dataset], 'vctk', 'noisy')
        test_path = os.path.join(default_config.datasets[args.dataset], 'vctk')
