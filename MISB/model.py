...
from .sdes import DiffusionBridgeSDE
from .loss import MelSpectrogramLoss, phase_loss, Loss_multi_discriminator, Loss_adv, GANLoss
from audiotools import AudioSignal
# from .gan_discriminator import MultiDiscriminator
from .dac_discriminator import Discriminator as MultiDiscriminator
...
class DiffusionBridge():
    def __init__(self, con=None, local_rank=0):
        ...
        if self.config.get("enable_gan_loss", False):
            self.md = MultiDiscriminator()
            self.md.to(self.device)
            self.optimizer_md = optim.Adam(self.md.parameters(), lr=self.config.learning_rate)
            self.gan_loss = GANLoss(self.md)
        ...
        self.sde = DiffusionBridgeSDE(schedule_type=self.config.bridge_type, t_max=self.config.t_max, loss_weight_type=self.config.loss_weight_type, device=self.device, method=self.config.get("method", "SB"))

        self.t_max, self.t_min = self.config.t_max, self.config.t_min

        if self.is_train and torch.distributed.is_initialized():
            self.generator = DDP(self.generator, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)
            if self.config.discriminator_training_strategy != 'none':
                self.discriminator = DDP(self.discriminator, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)
            if self.config.enable_gan_loss:
                self.md = DDP(self.md, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)
        ...
        # 加入损失函数
        if self.config.get("enable_mel_loss", False):
            self.multi_mel_fn = MelSpectrogramLoss(n_mels = [5, 10, 20, 40, 80, 160, 320],
    ...
    def save(self, epoch=None):
        ...
        if epoch is None:
            ...
            torch.save(self.state(), os.path.join(self.config.run_path, 'checkpoints', f'epoch_{epoch}.pt'))
        self.config.save()

    def restore(self):
        # checkpoint = torch.load(os.path.join(self.config.run_path, f'best.pt'), map_location="cpu")
        try:
            checkpoint_files = [f for f in os.listdir(os.path.join(self.config.run_path, 'checkpoints')) if f.startswith('epoch_') and f.endswith('.pt')]
            if checkpoint_files:
                # Extract epoch numbers and find the latest one
                latest_epoch = max([int(f.split('_')[1].split('.')[0]) for f in checkpoint_files])
                # latest_epoch = 200
                latest_checkpoint = os.path.join(self.config.run_path, 'checkpoints', f'epoch_{latest_epoch}.pt')
                print(latest_checkpoint)
                checkpoint = torch.load(latest_checkpoint, map_location="cpu")
            else:
                print(os.path.join(self.config.run_path, f'best.pt'))
                checkpoint = torch.load(os.path.join(self.config.run_path, f'best.pt'), map_location="cpu")
    ...
    def _generator_loss_fn(self):
        def _generator_loss_fn(x, y, x_bar=None):
            if self.config.get("fix_step", False):
                # 训练阶段使用与推理阶段相同的固定时间步序列
                timestep = torch.linspace(self.t_max, self.t_min, 16 + 1, device=x.device)[:-1]  # 去掉最后一个点，因为推理阶段通常是从t_max到t_min
                batch_indices = torch.randint(0, 16, (x.shape[0],), device=x.device)  # 随机选择4个时间步中的一个
                # batch_index = torch.randint(0, 4, (1,), device=x.device).item()  # 只生成一个随机索引
                # batch_indices = torch.full((x.shape[0],), batch_index, device=x.device)  # 广播到整个 batch            
                timestep = timestep[batch_indices]
            else:
                timestep = torch.rand(x.shape[0], device=x.device) * (self.t_max - self.t_min) + self.t_min
            

            # if x_bar is not None:
            #     x_bar_mean = torch.mean(x_bar)
            #     x_bar_std = torch.std(x_bar)
            #     print(f"x_bar mean: {x_bar_mean.item():.4f}, x_bar std: {x_bar_std.item():.4f}")
            if self.config.discriminator_training_strategy != 'none' and self.config.mean_inverting:
                ...
                print('这里目前不想看，等调通了再说吧。')
            elif self.config.discriminator_training_strategy == 'collaborative':
                # 注意这里x0是clean，x1是带噪语音
                if self.config.method == 'FM':
                    mean, std = (1-timestep)[:,None,None,None]*x + timestep[:,None,None,None]*y, \
                          (1-timestep)*self.sde.sigma_min + timestep*self.sde.sigma_max
                    z = torch.randn_like(x)  #
                    sigmas = std[:, None, None, None]
                    xt = mean + sigmas * z

                    if self.config.get("enable_fm_target", False):
                        label = x
                    else:
                        der_std = self.sde.sigma_max-self.sde.sigma_min
                        der_mean = y - x
                        label = (der_std * z + der_mean).detach()
                else:
                    xt = self.sde.q_sample(t=timestep, x0=x, x1=x_bar, ot_ode=self.config.ot_ode)
                    label = x #self.sde.compute_label(t=timestep, x0=x, xt=xt)
                print('collaborative 应该在这里。')
            else:
                if self.config.method == 'FM':
                    mean, std = (1-timestep)[:,None,None,None]*x + timestep[:,None,None,None]*y, \
                          (1-timestep)*self.sde.sigma_min + timestep*self.sde.sigma_max
            ...
            # 直接预测x0而不是score
            x0_pred = self.generator_fn(xt, timestep, cond=cond)

            complex_batch_loss = torch.square(torch.abs(x0_pred - label))
            ...
            # 多尺度mel loss
            audio_pred = STFTUtil.istft(x0_pred.squeeze(1).to(torch.float32)).unsqueeze(1)
            audio_label = STFTUtil.istft(label.squeeze(1).to(torch.float32)).unsqueeze(1)

            if self.config.enable_mel_loss:
                loss_mel = self.multi_mel_fn(AudioSignal(audio_pred.squeeze(1).to(torch.float32),16000), AudioSignal(audio_label.squeeze(1),16000)) * 15
                generator_loss += 0.1*loss_mel

            if self.config.enable_phase_loss:
            # # 相位loss
                pha = torch.angle(x0_pred.squeeze(1))
                pha_g = torch.angle(label.squeeze(1))
                L_IP, L_GD, L_PTD = phase_loss(pha, pha_g, 510, pha.size()[-1])
                # Losses defined on phase spectra
                L_P = L_IP + L_GD + L_PTD
                generator_loss += 0.01*L_P

            return generator_loss, (audio_pred, audio_label)
        ...
        def _step_fn(x, y):
            with self.autocast:
                if self.config.discriminator_training_strategy != 'none':
                    ...
                    if self.config.discriminator_training_strategy not in ['frozen_discriminator','collaborative']:
                        ...
                        print('计算判别式模型loss，权重是', self.config.discriminator_loss_weight)

                    generator_loss, audio_pair = self.generator_loss_fn(x, y=y, x_bar=x_bar)
                else:
                    generator_loss, audio_pair = self.generator_loss_fn(x, y=y)

                if self.config.discriminator_training_strategy not in ['none', 'frozen_discriminator','collaborative']:
                    return (1 - self.config.discriminator_loss_weight) * generator_loss + self.config.discriminator_loss_weight * discriminator_loss, audio_pair, {"train/generator_loss": generator_loss.item(), "train/discriminator_loss": discriminator_loss.item()}
                else:
                    return generator_loss, audio_pair, {"train/generator_loss": generator_loss.item()}
    ...
    def train(self):
        ...
        for epoch in range(self.config.current_epoch + 1, self.config.num_epoch):
            ...
            for clean_b, noisy_b in iter:
                ...
                loss, (esti_audio, target_audio), all_loss = self.step_fn(x, y)

                if self.config.enable_gan_loss:
                    loss_dis = self.gan_loss.discriminator_loss(esti_audio, target_audio.detach())
                    self.optimizer_md.zero_grad()
                    loss_dis.backward()
                    torch.nn.utils.clip_grad_norm_(self.md.parameters(), max_norm=5.0)
                    self.optimizer_md.step()

                    loss_adv, loss_fm = self.gan_loss.generator_loss(esti_audio, target_audio)

                    loss = loss + 10*loss_adv + 10*loss_fm
                else:
                    loss_adv, loss_dis, loss_fm = loss, loss, loss
                ...
                if (num_step + 1) % self.config.gradient_accumulation_steps == 0 or (num_step + 1) % len(train_dataloader) == 0:
                    ...
                    if self.local_rank == 0 and global_step % self.config.logging_per_step == 0:
                        ...
                        train_bar.update(epoch, train_loss_history, {'step': global_step, 'loss': scaler_format(loss.item(), 4), 'loss_dis': scaler_format(loss_dis.item(), 4),'loss_adv': scaler_format(loss_adv.item(), 4),'loss_fm': scaler_format(loss_fm.item(), 4)})
                        if self.config.wandb_log:
    ...
    def enhancement(self, audio, num_step=5, sampling_method='ddim', skip_type='time_uniform', ot_ode=False):
        if ot_ode is None:
        ...
        print(f"x mean: {x_mean.item():.4f}, x std: {x_std.item():.4f}")

        if self.config.discriminator_training_strategy == 'frozen_discriminator':
            xs = self.discriminator_fn(x)
            audio = invert_fn(xs)

            return audio, xs, xs
        
        #注意判别None和字符串
        if self.config.discriminator_training_strategy in ['None','none']:
            self.config.discriminator_training_strategy = None

        x_bar = None if self.config.discriminator_training_strategy is None else self.discriminator_fn(x)
        condition_map = {
            'none': [],
            'y': [x.clone()],
            'mean': [x_bar],
        ...
        elif sampling_method == 'fm':
            sampler = self.sde.get_fm_solver(model_fn=pred_x0_fn, num_step=num_step, ot_ode=ot_ode)
        else:
            raise NotImplementedError
        
        if self.config.discriminator_training_strategy!=None:
            xs, pred_x0s = sampler.sampling(x=x_bar, tp=self.config.get("enable_fm_target", False))
        else:
            xs, pred_x0s = sampler.sampling(x=x, tp=self.config.get("enable_fm_target", False))
