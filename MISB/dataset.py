...
class AudioFolder(Dataset):
    def __init__(self, audio_path_or_pt_file, sample_rate=16000, return_path=False, reverse=False):
        ...
        if isinstance(audio_path_or_pt_file, list):
            # Handle list of file paths (for DNS dataset)
            self.audio_paths = audio_path_or_pt_file
            self.sample_rate = sample_rate
        elif os.path.isdir(audio_path_or_pt_file):
        ...
        # else:
        #     raise NotImplementedError(f"Unsupported input type: {type(audio_path_or_pt_file)}")
...
class ComplexSpec(Dataset):
    def __init__(self, config, dataset='voicebank', subset='train', shuffle_spec=None, normalize_audio=None, return_raw = False, return_spec=True, dummy=False):
        ...
        assert dataset in ['voicebank','timit+wham', 'dns', 'farfield'], f'Dataset {dataset} is not supported yet.'
        ...
        if dataset == 'dns':
            noisy_dir = os.path.join(self.data_dir, subset, 'noisy')
            clean_dir = os.path.join(self.data_dir, subset, 'clean')
            self.noisy_files = sorted(glob(os.path.join(noisy_dir, '*.wav')))
            self.clean_files = [os.path.join(clean_dir, 'clean_fileid_' + os.path.basename(noisy_file).split('_fileid_')[-1]) 
                              for noisy_file in self.noisy_files]
            self.clean_files = AudioFolder(audio_path_or_pt_file=self.clean_files, sample_rate=self.sample_rate, return_path=True)
            self.noisy_files = AudioFolder(audio_path_or_pt_file=self.noisy_files, sample_rate=self.sample_rate, return_path=True)
        else:
            self.clean_files = AudioFolder(audio_path_or_pt_file=os.path.join(self.data_dir, subset, 'clean'), sample_rate=self.sample_rate, return_path=True)
            self.noisy_files = AudioFolder(audio_path_or_pt_file=os.path.join(self.data_dir, subset, 'noisy'), sample_rate=self.sample_rate, return_path=True)
    ...
    def __getitem__(self, i):
        x, x_path = self.clean_files[i]
        y, y_path = self.noisy_files[i]
